...
class FirebaseChatCore {
  ...
  /// custom data.
  Future<types.Room> createRoom({required String ownerId, required String docId, required String roomName, required String image, String? additionalMsg}) async {
    ...
    final roomQuery = await FirebaseFirestore.instance
        .collection('rooms')
        .where('id', isEqualTo: docId+(firebaseUser?.uid??''))
        .limit(1)
        .get();
    ...
    final userIds = [fu.uid, ownerId]..sort();
    ...
    final room = await FirebaseFirestore.instance
      ...
      'imageUrl': image,
      'name':  "${myOtherUser.firstName} ${myOtherUser.lastName} - $roomName",
      'updatedAt': FieldValue.serverTimestamp(),
      'userIds': userIds,
      'id':docId+(firebaseUser?.uid??''),
      "user":"${myCurrentUser.firstName} ${myCurrentUser.lastName}",
      "listID":docId,
    });
